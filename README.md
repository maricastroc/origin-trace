...
**Classify ([`src/engine/trace.ts`](src/engine/trace.ts)).** From "sourced at birth?" and "sourced now?" it derives the verdict — `born-sourced`, `retrofit`, or `unsourced-stable` (plus a _removed_ state) — assembles the timeline, and reports its confidence and caveats honestly in `meta.notes`.
...
| ------------- | --------------------------- | ------------------------------------------------------------------------- |
| **Quokka**    | _"happiest animal"_         | `retrofit` — asserted as fact years before any citation was attached.     |
