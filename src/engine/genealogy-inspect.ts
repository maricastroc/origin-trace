...
function printChain(label: string, lang: string, g: Genealogy): void {
  ...
  g.chain.forEach((hop, i) => {
    ...
    const dice =
      hop.overlap !== undefined ? `  dice=${hop.overlap.toFixed(2)}` : "";
    const link = hop.anchorsShared.length
      ? `  →shared[${hop.anchorsShared.join(", ")}]`
      : "";
  ...
  if (g.stopOverlap !== null) {
    process.stdout.write(
      `  ⨯ abstained: next link dice=${g.stopOverlap.toFixed(2)} below threshold\n`,
    );
  }
  process.stdout.write(
    `  verify:\n` +
...
async function main(argv: string[]): Promise<number> {
  ...
  if (articles.length === 0) {
    process.stderr.write(
      'Usage: genealogy-inspect.ts --articles "A,B" [--shifts] [--deep N] [--sample N]\n',
    );
    return 2;
  }
