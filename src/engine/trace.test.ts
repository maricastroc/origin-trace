...
describe("traceClaim — verdicts", () => {
  ...
  it("classifies a claim that gained its citation later as retrofit", async () => {
    const bare =
      "The quokka is widely called the happiest animal on the earth.";
    const cited =
  ...
  it("flags citogenesis when the retrofitted source postdates the claim", async () => {
    const bare =
      "The coati is affectionately known as a Brazilian aardvark by locals.";
    const cited =
  ...
  it("classifies a never-cited but never-removed claim as unsourced-stable", async () => {
    const bare =
      "The subject has an unusually complicated administrative boundary here.";
    const fetchJson = history([
  ...
  it("classifies a claim born cited then stripped of its citation as source-lost", async () => {
    ...
    const present = [...prov.timeline]
      .reverse()
      .find((e) => e.kind !== "claim-absent");
  ...
  it("records a removal when the claim is gone from the latest revision", async () => {
    const withClaim =
      "The stadium once held the outright world attendance record for football.";
    const fetchJson = history([
      { revid: 1, timestamp: "2010-01-01T00:00:00Z", content: UNRELATED },
  ...
  it("abstains from asserting an origin it can't confirm (low-overlap reword)", async () => {
    const fetchJson = history([
      { revid: 1, timestamp: "2008-01-01T00:00:00Z", content: UNRELATED },
      {
        ...
        content:
          "In 1838 William Whewell recruited Darwin as society secretary.",
      },
      {
        revid: 3,
        timestamp: "2014-01-01T00:00:00Z",
        content: "In 1838 he went geologising across the Scottish highlands.",
      },
    ]);
...
describe("traceClaim — origin honesty (proven vs. sampled first occurrence)", () => {
  it("presents a fully-read origin as the PROVEN first occurrence", async () => {
    ...
    const prov = await traceClaim({
      article: "Subject",
      phrase: "established on the ridge in 1888",
      fetchJson,
    });
  ...
  it("does NOT claim proof when a sparse earlier occurrence below the origin can't be ruled out", async () => {
    ...
    const revs: FakeRevision[] = Array.from({ length: 1000 }, (_, i) => ({
      revid: 1000 + i,
      parentid: i === 0 ? 0 : 1000 + i - 1,
      timestamp: new Date(Date.UTC(2000, 0, 1) + i * 3 * 86_400_000)
        .toISOString()
        .replace(/\.\d{3}Z$/, "Z"),
      content: present(i)
        ? `Overview here. General context stays constant. The ${CLAIM} appears in this revision. Discussion follows.`
        : `Overview here. General context stays constant. Discussion follows.`,
    }));
