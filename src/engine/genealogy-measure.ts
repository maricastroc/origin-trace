...
async function representativeCorpus(
  ...
  for (const article of articles) {
    try {
      ...
      const { picks, candidates } = sampleBodyClaims(
        current.content,
        perArticle,
      );
      picks.forEach((raw, k) =>
        claims.push({ label: `${article}#${k}`, article, phrase: raw, lang }),
      );
      process.stdout.write(
        `  · ${article}: ${picks.length} claims (of ${candidates} body candidates)\n`,
      );
    } catch (err) {
  ...
  for (const claim of corpus) {
    ...
    if (i % 5 === 0 || i === corpus.length) {
      const snapshot = [...running.entries()]
        ...
        .join(" · ");
      process.stdout.write(
        `  [${corpusName} ${i}/${corpus.length}] ${snapshot}\n`,
      );
    }
  }
...
function report(name: string, results: Result[]): Record<string, unknown> {
  const located = results.filter(
    (r): r is Result & { g: Genealogy } => r.g !== null,
  );
  ...
  const pct = (k: number) =>
    n ? `${Math.round((k / n) * 100)}%`.padStart(4) : "  —";
  ...
  for (const s of SHAPES) {
    ...
    if (c)
      process.stdout.write(
        `    ${s.padEnd(26)} ${String(c).padStart(3)} ${pct(c)}\n`,
      );
  }
  ...
  for (const t of TERMINI) {
    ...
    for (const { g } of hits) {
      const arrow = g.movedEarlier
        ? `${g.lexicalOrigin.date}→${g.origin.date}`
        : `${g.origin.date}`;
      ...
      const shown = g.chain[0]?.wording ?? g.phrase;

      process.stdout.write(
        `      • ${g.article} — "${truncate(shown, 56)}"  (${arrow}, ${g.chain.length}h)${shift}\n`,
      );
    }
  }
  ...
  if (missed.length) {
    ...
    for (const r of missed.slice(0, 8)) {
      process.stdout.write(
        `    · ${r.claim.article} — "${truncate(r.claim.phrase, 56)}"\n`,
      );
    }
  }

  const shapePct = (s: Shape) =>
    n ? Math.round(((byShape.get(s) ?? 0) / n) * 100) : 0;
  process.stdout.write(
    `\n  conclusion\n` +
...
async function main(argv: string[]): Promise<number> {
  ...
  if (only !== "representative") {
    dump.push(
      report("curated", await measure(curatedCorpus(), cache, "curated", out)),
    );
  }
  if (only !== "curated") {
    ...
    dump.push(
      report(
        "representative",
        await measure(corpus, cache, "representative", out),
      ),
    );
  }
